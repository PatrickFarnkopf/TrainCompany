...
<div id="groupList">
	<ul>
		...
		<li>&middot;</li>
		<li><a href="<?= cml('game_tasks_active', array(), 'groupList') ?>">Aktive Ausschreibungen</a></li>
	</ul>
</div>

<form method="post" action="<?= cml('game_tasks',array('makeAction'=>true)) ?>">
	<? if(count(cmi()->getVarCache('taskList')) == 0): ?>
		<p class="Center">
			Derzeit existieren keine Ausschreibungen, die du erfüllen könntest.
		</p>
	<? endif; ?>
	<? foreach(cmi()->getVarCache('taskList') as $taskID => $currentTask): ?>
		<div class="Task">
			...
			<span class="Right">
				<? if($currentTask->getType() == Task::WITH_APPLICATION): ?>
					<? if($currentTask->countApplicationsWithoutUser(lsi()->getUserInstance()) == 1): ?>
						<em>(ein anderer)</em>
					<? else: ?>
						<em>(<?= Format::number($currentTask->countApplicationsWithoutUser(lsi()->getUserInstance())) ?> andere)</em>
					<? endif; ?>
					<? if($currentTask->existApplicationForUser(lsi()->getUserInstance())): ?>
