...
<div id="groupList">
	<ul>
		<? $i = 0 ?>
		<? foreach(\Core\i::Module()->getVarCache('settingModules') as $settingLink => $settingName): ?>
			<? $i ++ ?>
			<? if($i > 1): ?>
				<li>&middot;</li>
			<? endif; ?>
			<li>
				<? if($settingLink == \Core\i::Module()->getVarCache('currentModule')): ?>
					&raquo;<?= Format::string($settingName) ?>&laquo;
				<? else: ?>
					<a href="<?= \Core\Module::createModuleLink($settingLink, array(),'groupList') ?>"><?= Format::string($settingName) ?></a>
				<? endif; ?>
			</li>
		<? endforeach; ?>
	</ul>
