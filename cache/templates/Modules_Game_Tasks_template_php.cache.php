...
<div id="groupList">
	<ul>
		...
		<li>&middot;</li>
		<li><a href="<?= \Core\Module::createModuleLink('Game_Tasks_Active', array(), 'groupList') ?>">Aktive Ausschreibungen</a></li>
	</ul>
</div>

<form method="post" action="<?= \Core\Module::createModuleLink(NULL ,array('makeAction'=>true)) ?>">
	<? if(count(\Core\i::Module()->getVarCache('taskList')) == 0): ?>
		<p class="Center">
			Derzeit existieren keine Ausschreibungen, die du erfüllen könntest.
		</p>
	<? endif; ?>
	<? foreach(\Core\i::Module()->getVarCache('taskList') as $taskID => $currentTask): ?>
		<div class="Task">
			...
			<span class="Right">
				<? if($currentTask->getType() == \Game\Task::WITH_APPLICATION): ?>
					<? if($currentTask->countApplicationsWithoutUser(\Core\i::Module()->getVarCache('currentUserInstance')) == 1): ?>
						<em>(ein anderer)</em>
					<? else: ?>
						<em>(<?= Format::number($currentTask->countApplicationsWithoutUser(\Core\i::Module()->getVarCache('currentUserInstance'))) ?> andere)</em>
					<? endif; ?>
					<? if($currentTask->existApplicationForUser(\Core\i::Module()->getVarCache('currentUserInstance'))): ?>
