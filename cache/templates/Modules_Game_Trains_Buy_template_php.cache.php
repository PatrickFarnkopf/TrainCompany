<?php use \Core\Format; ?><a href="<?= \Core\Module::createModuleLink('Game_Trains') ?>">&laquo; Zurück zu deinen Fahrzeugen</a>
...
<div id="groupList">
	<ul>
		<? $i = 0 ?>
		<? foreach(\Core\i::Module()->getVarCache('trainTypes') as $trainType => $typeName): ?>
			<? $i ++ ?>
			<? if($i > 1): ?>
				<li>&middot;</li>
			<? endif; ?>
			<li>
				<? if(\Core\i::Module()->getVarCache('trainType') == $trainType): ?>
					&raquo;<?= Format::string($typeName) ?>&laquo;
				<? else: ?>
					<a href="<?= \Core\Module::createModuleLink(NULL, array('trainType'=>$trainType),'groupList') ?>"><?= Format::string($typeName) ?></a>
				<? endif; ?>
			</li>
		<? endforeach; ?>
	</ul>
...
<form method="post" action="<?= \Core\Module::createModuleLink(NULL,array('trainType'=>\Core\i::Module()->getVarCache('trainType'),'buyTrains'=>true)) ?>">
	<table class="OverviewTable">
		...
		<? foreach(\Core\i::Module()->getVarCache('trainList') as $key=>$currentTrain): ?>
		<tr class="Table<?= $key%2 ?>">
			<td>
				<span>
					<?= Format::string($currentTrain->getName()) ?>
					<div class="DetailView">
						<? \Core\i::Module()->includeTemplate('currentTrain',array('train'=>$currentTrain)) ?>
					</div>
				</span>
			</td>
			...
			<td class="Center"><?= Format::number(\Core\i::Module()->getVarCache('currentUserInstance')->searchTrain($currentTrain)) ?>+<input type="text" size="2" placeholder="0" name="trains[<?= $currentTrain->getID() ?>]"></td>
		</tr>
		<? endforeach; ?>
	</table>
